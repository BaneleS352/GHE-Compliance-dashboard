...
import { getWorkflowForDeclaration, getConfig } from "../../data/db";
...
export function DeclarationDetailView({
  ...
  const isDeclaration = typeof (data as Declaration).value === "number";
  const d = isDeclaration ? (data as Declaration) : null;
  const record = !d ? (data as Record<string, string>) : null;

  const workflow = d ? getWorkflowForDeclaration(d.id) : null;
  ...
  const fields: [string, string][] = d
    ? [
        ...
        ...(d.value > getConfig().highValueThreshold ? ([["Substantiation (> R2 500)", safe(d.substantiation || "Required")]] as [string, string][]) : []),
      ]
    : [
        ...
        ...(Number(record?.value) > getConfig().highValueThreshold ? ([["Substantiation (> R2 500)", safe(record?.substantiation || "Required")]] as [string, string][]) : []),
      ];
  ...
  const fixedSlots = [
    { label: "Submission", role: null },
    { label: "Line Manager Review", role: "lineManager" },
    { label: "HR Review", role: "hr" },
  ...
  const workflowSteps = workflow
    ? fixedSlots.map((slot) => {
        if (!slot.role) {
          return {
            label: "Submission",
            actor: d?.employee || "Team Member",
            done: true,
            notRequired: false,
            updates: [{ status: "Submitted", date: d ? d.submitted : "-", time: "-" }],
          };
        }
        const step = workflow.steps.find((s) => s.role === slot.role);
        if (!step) {
          return {
            label: slot.label,
            actor: "—",
            ...
            notRequired: true,
            updates: [{ status: "Not Required", date: "-", time: "-" }],
          };
        }
        return {
          label: step.label,
    ...
    : [
        { label: "Submission", actor: d?.employee || "Team Member", done: true, notRequired: false, updates: [{ status: "Submitted", date: d ? d.submitted : "-", time: "-" }] },
        { label: "Line Manager Review", actor: "—", done: false, notRequired: true, updates: [{ status: "N/A", date: "-", time: "-" }] },
        { label: "HR Review", actor: "—", done: false, notRequired: true, updates: [{ status: "N/A", date: "-", time: "-" }] },
        { label: "CEO Approval", actor: "—", done: false, notRequired: true, updates: [{ status: "N/A", date: "-", time: "-" }] },
      ];
    ...
    <div className="space-y-5">
      ...
      <div className="xl:col-span-5 flex flex-col xl:flex-row gap-5">
        ...
        >
          ...
          <Card className="p-6 h-full rounded-2xl bg-gradient-to-br from-[#f8fafc] via-[#eef2ff] to-[#e0e7ff] border border-white/40 shadow-[0_10px_30px_rgba(0,0,0,0.08)] backdrop-blur-sm relative overflow-hidden">
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(99,102,241,0.15),transparent_60%)] pointer-events-none" />
            <div className="absolute bottom-0 right-0 opacity-20 pointer-events-none">
              <div className="w-32 h-32 bg-indigo-300 rounded-full blur-2xl" />
            </div>
            <div className="relative z-10">
              <h2 className="text-xs font-semibold tracking-wide text-slate-500 uppercase mb-6">Declaration Details</h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {fields.map(([k, v]) => (
                  <motion.div key={k} whileHover={{ scale: 1.015, y: -2 }} transition={{ duration: 0.2 }}
                    className={`rounded-xl p-4 bg-white/70 backdrop-blur-sm border border-white/60 shadow-sm transition-all hover:shadow-md ${["Description", "Substantiation (> R2 500)"].includes(k) ? "sm:col-span-2" : ""}`}
                  >
                    <p className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">{k}</p>
                    <p className="mt-2 text-sm font-medium text-slate-800 break-words">{v}</p>
                  </motion.div>
                ))}
              </div>
            </div>
        ...
        <div className="flex-[2]">
          ...
          >
            {/* Decorative glow */}
          <Card className="p-6 h-full flex flex-col rounded-2xl bg-gradient-to-br from-[#f8fafc] via-[#eef2ff] to-[#e0e7ff] shadow-[0_10px_30px_rgba(0,0,0,0.08)] border border-white/40 backdrop-blur-sm relative overflow-hidden">
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(99,102,241,0.15),transparent_60%)] pointer-events-none" />
            <div className="absolute bottom-0 right-0 opacity-20 pointer-events-none">
              <div className="w-32 h-32 bg-indigo-300 rounded-full blur-2xl" />
            </div>
            <div className="relative z-10 flex flex-col h-full">
              ...
              <h3 className="text-xs font-semibold tracking-wide text-gray-500 uppercase mb-6">Approval Workflow</h3>
              <div className="relative flex flex-col gap-6 flex-1">
                <div className="absolute left-4 top-2 bottom-2 w-[2px] bg-gray-300/60" />
                <motion.div className="absolute left-4 top-2 w-[2px] bg-gradient-to-b from-emerald-500 to-emerald-400"
                  initial={{ height: 0 }}
                  animate={{ height: `${((workflowSteps.filter((s) => s.done || s.notRequired).length - 1) / (workflowSteps.length - 1 || 1)) * 100}%` }}
                  transition={{ duration: 0.6, ease: "easeOut" }}
                />

                {workflowSteps.map((step, i) => {
                  const activeSteps = workflowSteps.filter((s) => !s.done && !s.notRequired);
                  const isActive = !step.done && !step.notRequired && activeSteps.length > 0 && workflowSteps.findIndex((s) => !s.done && !s.notRequired) === i;
                  return (
                    <motion.div key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: i * 0.05 }} whileHover={{ scale: 1.01 }} className="flex items-start gap-4 relative">
                      <div className="relative z-10">
                        <motion.div whileHover={{ scale: 1.1 }}
                          className={`w-8 h-8 flex items-center justify-center rounded-full text-xs font-semibold transition-all ${
                            step.done ? "bg-emerald-500 text-white shadow" :
                            step.notRequired ? "bg-gray-200 text-gray-400 border border-gray-300" :
                            isActive ? "bg-white border-2 border-emerald-500 text-emerald-600 shadow-[0_4px_20px_rgba(99,102,241,0.25)]" :
                            "bg-white/40 border border-white/50 text-gray-400"
                          }`}
                        >
                          {step.done ? <Check size={14} /> : step.notRequired ? "—" : i + 1}
                        </motion.div>
                      </div>
                      ...
                      <motion.div
                        ...
                        `}
                      >
                      <motion.div layout className={`flex-1 rounded-xl p-4 transition-all ${isActive ? "bg-white/70 backdrop-blur-sm border border-white/60 shadow-sm" : ""}`}>
                        <div className="flex items-center justify-between">
                          <p className={`text-sm font-semibold ${step.done ? "text-gray-800" : isActive ? "text-indigo-700" : "text-gray-400"}`}>{step.label}</p>
                          {step.done && <span className="text-[10px] px-2 py-1 rounded-full bg-emerald-100 text-emerald-600">Completed</span>}
                          {isActive && <span className="text-[10px] px-2 py-1 rounded-full bg-indigo-100 text-indigo-600">In Progress</span>}
                          {step.notRequired && <span className="text-[10px] px-2 py-1 rounded-full bg-gray-100 text-gray-400">Not Required</span>}
                        </div>
                        ...
                        {isActive && (
                          ...
                          >
                            ...
                            <b>{step.actor}</b>
                          <motion.div initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} className="mb-3 p-3 rounded-lg bg-indigo-50 border border-indigo-100 text-xs">
                        ...
                        <div className="space-y-2">
                          {step.updates.map((u, idx) => (
                            ...
                            <div key={idx} className="text-[11px] rounded-lg border border-white/60 bg-white/60 backdrop-blur-sm px-3 py-2">
                              <div className="flex justify-between"><span className="text-gray-500">Status</span><span className="font-medium">{u.status}</span></div>
                              <div className="flex justify-between"><span className="text-gray-500">Date</span><span>{u.date}</span></div>
                              <div className="flex justify-between"><span className="text-gray-500">Time</span><span>{u.time}</span></div>
                            </div>
                          ))}
                        </div>
                      </motion.div>
      ...
      <div className="xl:col-span-3">
        ...
        >
          {/* Glow */}
        <Card className="p-6 rounded-2xl bg-gradient-to-br from-[#f8fafc] via-[#eef2ff] to-[#e0e7ff] border border-white/40 shadow-[0_10px_30px_rgba(0,0,0,0.08)] backdrop-blur-sm relative overflow-hidden">
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(99,102,241,0.15),transparent_60%)] pointer-events-none" />
          <div className="absolute bottom-0 right-0 opacity-20 pointer-events-none">
            <div className="w-32 h-32 bg-indigo-300 rounded-full blur-2xl" />
          </div>
          <div className="relative z-10">
            ...
            <h3 className="text-xs font-semibold tracking-wide text-slate-500 uppercase mb-6">Supporting Documents</h3>
            <div className="space-y-3">
              {(record?.files ?? "").split(",").filter(Boolean).map((file, i) => (
                <motion.button key={i} whileHover={{ scale: 1.02, y: -2 }} whileTap={{ scale: 0.98 }}
                  onClick={() => window.open(file.trim(), "_blank")}
                  className="w-full rounded-xl border border-white/60 bg-white/70 backdrop-blur-sm px-4 py-3 flex items-center justify-between transition-all hover:shadow-md"
                >
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-indigo-100 flex items-center justify-center text-lg">📄</div>
                    <div className="text-left">
                      <p className="font-medium text-sm text-slate-800">{file.trim()}</p>
